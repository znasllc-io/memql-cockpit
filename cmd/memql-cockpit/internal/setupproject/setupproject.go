// Package setupproject implements `memql-cockpit setup project` -- the
...
//     what CI exercises.
//
//     memql-cockpit setup project --product=acme --product-org=acme-io \
...
// `memql-cockpit setup ...`. Returns the process exit code.
...
// runWizard spins up a tcell screen, drives the wizard, and tears the screen
// down before returning so the subsequent flow can stream bootstrap output to a
// normal terminal.
func runWizard(seed Config) (Config, Outcome, int) {
	screen, err := ui.NewScreen()
	if err != nil {
		fmt.Fprintf(os.Stderr, "ERROR: opening terminal for the wizard: %v\n", err)
		return seed, OutcomeCanceled, exitFailure
	}
	cfg, outcome := RunWizard(screen, ui.DefaultTheme(), seed)
	screen.Fini()
	return cfg, outcome, exitOK
}
...
func printSetupUsage() {
	fmt.Println("Usage: memql-cockpit setup <target> [flags]")
	fmt.Println()
	fmt.Println("Targets:")
	fmt.Println("  project   Stamp a new memQL product workspace from the memql-project template")
	fmt.Println()
	fmt.Println("Run `memql-cockpit setup project --help` for the project flags.")
}

func printProjectUsage() {
	fmt.Println("Usage: memql-cockpit setup project [flags]")
	fmt.Println()
