...
func printUsage() {
	...
	fmt.Println()
	fmt.Println("Examples (plan ids are moving to bare shortIds -- the bare-ids client contract):")
	fmt.Println("  memql-cockpit harness trace a9f3b7c2                    # bare shortId (going-forward form)")
	fmt.Println("  memql-cockpit harness trace v1:planner:plan:a9f3b7c2    # canonical form the engine matches today")
	fmt.Println("  memql-cockpit harness trace a9f3b7c2 --cluster staging")
	fmt.Println()
	fmt.Println("harness trace forwards the id to the engine unchanged (no client-side")
	fmt.Println("canonicalization). The engine's harnessTrace lookup currently matches the")
	fmt.Println("stored CANONICAL plan id, so bare-id resolution for this path is a tracked")
	fmt.Println("#2441/#2444 follow-up -- pass the canonical id the app prints until then.")
}
