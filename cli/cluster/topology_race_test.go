...
// TestPartitions_NoRaceUnderConcurrentMutate mirrors the topology
// race test for the partitions pane. Same shape: background
// subscriber goroutine vs UI reader. Without locking the test
// would tear v.Partitions mid-iteration; with the lock it's clean.
func TestPartitions_NoRaceUnderConcurrentMutate(t *testing.T) {
	v := NewPartitionsView(ui.DefaultTheme())
	v.SetPartitions([]PartitionInfo{
		{Name: "default", PartitionType: "standard"},
		{Name: "personal", PartitionType: "personal"},
	})

	const iters = 1000
	var wg sync.WaitGroup

	// Writer: churn the partition list.
	wg.Add(1)
	go func() {
		defer wg.Done()
		for i := 0; i < iters; i++ {
			if i%2 == 0 {
				v.SetPartitions([]PartitionInfo{
					{Name: "default", PartitionType: "standard"},
					{Name: "personal", PartitionType: "personal"},
					{Name: "acme", PartitionType: "dedicated"},
				})
			} else {
				v.SetPartitions([]PartitionInfo{
					{Name: "default", PartitionType: "standard"},
	...
	// Writer 2: bang on Reset which clears the slice. The screenshot
	// race was specifically "between SetPartitions clearing and
	// re-filling", so this is the most aggressive mutation pattern.
	wg.Add(1)
	go func() {
		defer wg.Done()
		for i := 0; i < iters; i++ {
	...
	// Readers: walk the partition list under the RLock.
	wg.Add(1)
	go func() {
		defer wg.Done()
		for i := 0; i < iters*2; i++ {
	...
	// FormOpen: takes the lock itself, exercises that path too.
	wg.Add(1)
	go func() {
		defer wg.Done()
		for i := 0; i < iters; i++ {
